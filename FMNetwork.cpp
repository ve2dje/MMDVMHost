...
bool CFMNetwork::writeData(float* data, unsigned int nSamples)
{
	...
	for (long i = 0L; i < nSamples; i++) {
		unsigned short val = (unsigned short)((data[i] ) * 32767.0F );  // Changing from U16BE to S16LE
#else
	for (long i = 0L; i < src.output_frames_gen; i++) {
		unsigned short val = (unsigned short)((src.data_out[i] ) * 32767.0F ); // Changing from U16BE to S16LE
#endif

		buffer[length++] = (val >> 0) & 0xFFU; // Changing from U16BE to S16LE
		buffer[length++] = (val >> 8) & 0xFFU; // Changing from U16BE to S16LE
	}
...
void CFMNetwork::clock(unsigned int ms)
{
	...
	//	LogMessage("FM packet received from an invalid source"); // removed , problem  with USRP audio. Needs more brain
	//	return;
	//}
...
unsigned int CFMNetwork::read(float* data, unsigned int nSamples)
{
	...
	if (m_sampleRate != 8000U) {
		...
		for (unsigned int i = 0U; i < nSamples; i++) {
			unsigned short val = ((buffer[i * 2U + 0U] & 0xFFU) << 0) +  // Changing from U16BE to S16LE
			                     ((buffer[i * 2U + 1U] & 0xFFU) << 8);	// Changing from U16BE to S16LE
			in[i] = (float(val) / 65536.0F );	// Changing from U16BE to S16LE
		}
		...
		for (unsigned int i = 0U; i < nSamples; i++) {
			unsigned short val = ((buffer[i * 2U + 0U] & 0xFFU) << 0) +	// Changing from U16BE to S16LE
			                     ((buffer[i * 2U + 1U] & 0xFFU) << 8);	// Changing from U16BE to S16LE
			data[i] = (float(val)  / 65536.0F );	// Changing from U16BE to S16LE
		}
